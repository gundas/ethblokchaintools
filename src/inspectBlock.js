...
if (process.argv.length != 4) {
   console.log("Usage: ");
   console.log("node inspectBlock.js <start block Number> <finish block number>");
   process.exit(1);
}
   ...
   getBlockContracts(w3, blockNumber).then( contracts => {
         if (contracts && contracts.length) {
             for (var contract of contracts) { 
               console.log("%s %d %d", contract.address, w3.utils.fromWei(contract.balance), blockNumber);
             }
         }
   ...
   if (block && block.transactions) {
      for (var tran of block.transactions) {
         ...
         if (contractAddr) {
            ...
            contracts.push({'address':contractAddr, 'balance':balance});
         }
      }
   }
   return contracts;
